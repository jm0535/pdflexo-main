...
const PDFSidebar: React.FC = () => {
  ...
  const { expanded, isMobileView, isOpen, toggleExpanded, setExpanded } =
    useSidebar();
  const [isDark, setIsDark] = useState(false); // Initialize with a default value

  // Force sidebar to be expanded on mount
  useEffect(() => {
    // Force the sidebar to be expanded
    setExpanded(true);
  }, [setExpanded]);
  ...
  const handleThemeToggle = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("themeChanged", { detail: { isDark: newIsDark } })
    );
  };
      ...
      <Sidebar>
        <SidebarHeader>
          <Link to="/" className="flex items-center justify-center">
            <FileText
              className={`${
                expanded || isMobileView ? "w-6 h-6" : "w-8 h-8"
              } text-[#9b87f5] dark:text-[#8B5CF6]`}
            />
          </Link>
          <Button
            variant="ghost"
                  ...
                  <SidebarMenuItem key={item.path}>
                    <SidebarMenuButton asChild>
                      ...
                      >
                        <item.icon
                          className={`${
                            expanded || isMobileView ? "w-5 h-5" : "w-5 h-5"
                          } text-[${item.color}]`}
                        />
                        ...
                        >
                          ...
                          {true && (
                            <div className="flex items-center">
                              <span>{item.title}</span>
                              <Badge className="ml-2 text-[0.6rem] px-1 py-0 h-4 bg-orange-500 text-white border-orange-600">
                                SOON
                              </Badge>
                            </div>
                          )}
