...
const SplitPDF = () => {
  ...
  const handleAddSplitPoint = () => {
    ...
    if (isNaN(pageNum) || pageNum < 1 || (document && pageNum > document.totalPages)) {
      toast.error(`Please enter a valid page number between 1 and ${document?.totalPages || 'max'}`);
      return;
    }
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-card rounded-lg shadow-sm p-6 mb-6 border border-border">
            ...
            <p className="text-muted-foreground mb-6">
              Upload a PDF file and specify the page numbers where you want to split the document into separate files.
            </p>
              ...
              <div className="space-y-6">
                <div className="p-4 bg-secondary rounded-md flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{document.totalPages} pages</p>
                  </div>
                ...
                <div className="space-y-4">
                  <h3 className="text-lg font-medium text-foreground">Add Split Points</h3>
                  <p className="text-sm text-muted-foreground">
                    Enter the page numbers where you want to split the document. For example, entering "3" will create a split before page 3.
                  </p>
                  ...
                  {splitPoints.length > 0 && (
                    <div className="mt-4">
                      <h4 className="font-medium mb-2 text-foreground">Current Split Points:</h4>
                      <div className="flex flex-wrap gap-2">
                        {splitPoints.map(point => (
                          <div key={point} className="bg-secondary px-3 py-1 rounded-full flex items-center">
                            <span className="text-foreground">Page {point}</span>
                            <button 
          ...
          {splitPoints.length > 0 && document && (
            <div className="bg-card rounded-lg shadow-sm p-6 border border-border">
              <h3 className="text-lg font-medium mb-4 text-foreground">Preview of Split Files</h3>
              <div className="space-y-3">
                <div className="p-3 border border-border rounded-md">
                  <p className="font-medium text-foreground">File 1</p>
                  <p className="text-sm text-muted-foreground">Pages 1 to {splitPoints[0] - 1}</p>
                </div>
                
                {splitPoints.map((point, index) => (
                  <div key={point} className="p-3 border border-border rounded-md">
                    <p className="font-medium text-foreground">File {index + 2}</p>
                    <p className="text-sm text-muted-foreground">
                      Pages {point} to {splitPoints[index + 1] ? splitPoints[index + 1] - 1 : document.totalPages}
                    </p>
                  </div>
                ))}
              </div>
